                                ...
                                <el-row>
                                    ...
                                    <el-col :span="8">
                                        <el-form-item label="所属学科 :" prop="subjectid" class="is-required">
                                            <el-select v-model="form.subjectid" @change="onChangeSubject(form.schoolsectionid,form.subjectid)">
                                                <el-option :label="item.dictname" :value="item.dictvalue" v-for="(item, idx) in subjectitems" :key="idx" :data="item"></el-option>
                                            </el-select>
                                        </el-form-item>
                                    </el-col>
                                    <el-col :span="8">
                                        <el-form-item label="所属年级 :" prop="gradeid" class="is-required" @change="onChangeGrade()">
                                ...
                                <el-row>
                                    <el-col :span="8">
                                        <el-form-item label="教材版本:" prop = "editiontypeid" class = "is-required">
                                            <el-select v-model = "form.editiontypeid" @change="onChangeEditionType(form.schoolsectionid, form.subjectid,form.editiontypeid)">                                   
                ...
                <div class="ecrw-zhengji-upload-right">
                    <div class="ecrw-zhengji-upload-right-wrap">
                        <div class="ecrw-zhengji-upload-user">
                            <div class="ecrw-zhengji-upload-user-left" v-if="userInfo!=null">
                                <ul>
                            ...
                            <div class="ecrw-zhengji-upload-user-right" v-if="userInfo!= null">
                                <ul>
                                    <li class="ecrw-zhengji-upload-user-text">
                                        参与征集：<span class="ecrw-color-red">{{userInfo.collectionjoincount}}</span>次
                                    </li>
                                    
                                    <li class="ecrw-zhengji-upload-user-text">
                                        参与悬赏：<span class="ecrw-color-red">{{userInfo.rewardjoincount}}</span>次
                                    </li>
                                    <li class="ecrw-zhengji-upload-user-text">
                                        参与评比：<span class="ecrw-color-red">{{userInfo.estimatejoincount}}</span>次
                                    </li>
                                    <li class="ecrw-zhengji-upload-user-text">
                                        我发起的悬赏：<span class="ecrw-color-red">{{userInfo.rewardcreatecount}}</span>次
                                    </li>
                                </ul>
                            </div>
                        </div>
...
<script>
    ...
    import { getSubjectList,  getSchoolSectionList, getNaviGradeList,getNaviEditionTypeList, getTeachCatalogList,getNaviBookModelList,uploadResource, getTeachCatalogHeader, getCatalogList, uploadFile, uploadFileInfo, uploadEstimateFileInfo, uploadEstimateResource, getActivityResourceList, getUserCountInfo, getEstimateActivity, getFileList } from '../../api/api';
        ...
        data() {
            return {
                ...
                formRules: {
                    ...
                    editiontypeid: [
                        { required: true, message: '请选项教材版本', trigger: 'blur' },
                    ],
                    bookmodelid: [
                        { required: true, message: '请选项册别', trigger: 'blur' },
                    ]
                },
        ...
        methods: {
            validateNameProperty(rule, value, callback) {
                ...
                if (filename.length > 25) {
                    callback(new Error('文件名不能超过50文字'));
            ...
            getUserCountInfo() {
                getUserCountInfo(this.loginInfo.userid).then((res) => {
                    if (res.data.serverResult.resultCode == "200") {
                        this.userInfo = res.data.responseEntity;
                    } else {
                        this.$message({
                            message: res.data.serverResult.resultMessage,
                            type: 'error'
            ...
            getNaviGrade(schoolsectionid) {
             ...
             getNaviEditionType(schoolsectionid, subjectid) {
                let param = {
            ...
            getNavBookModel(schoolsectionid, subjectid, editiontypeid) {
                let param = {
                    schoolsectionid:schoolsectionid,
                    subjectid:subjectid,
            ...
            addProperty: function() {
                ...
                this.form.chapterPath[this.curChapterIndex] = this.thread;
            ...
            onChangeSubject(schoolsectionid,subjectid) {
                this.form.chapterPath = [];
                this.getNaviEditionType(schoolsectionid,subjectid);
            },            
            onChangeGrade() {
                this.form.chapterPath = [];
            },
            onChangeEditionType(schoolsectionid,subjectid,editiontypeid) {
                this.form.chapterPath = [];
                this.getNavBookModel(schoolsectionid,subjectid,editiontypeid);
            },
            onChangeBookModel(){
                this.form.chapterPath = [];
            },
            ...
            doUpload: function() {
                this.$refs.form.validate((valid) =>{
                    if(valid) {
                        ...
                        uploadEstimateResource(para).then((res) => {
                            if (res.data.serverResult.resultCode == "200") {
            ...
            uploadFileInfo: function(contentId, fileInfo) {
                ...
                uploadFileInfo(para).then((res) => {
                    if (res.data.serverResult.resultCode == "200") {
                        ...
                        //==== kmh->bug number: 548->start date: Jan 26
                        console.log("Upload done!");
                        setTimeout(function(){
                            window.history.go(-1);
                        }, 2000);
                        //=== kmh->bug number: 548->end date: Jan 26
